...
*   **[Action Filter](./action-filter.mdx)**: Filters by the type of event (e.g., breaking a block, adding an item).
*   **[Exclude Filter](./exclude-filter.mdx)**: Excludes specific blocks or items from the results.
*   **[Include Filter](./include-filter.mdx)**: Restricts the search to only the specified blocks or items.
*   **[Radius Filter](./radius-filter.mdx)**: Limits the search to a cubic area around you.
*   **[Time Filter](./time-filter.mdx)**: Searches for logs within a specific recent timeframe.
*   **[User Filter](./user-filter.mdx)**: Filters by the player who performed the action.```